...
do
	...
	function ChangeTeam(Team)
		if Team == game.Teams.Zeykoz then
			local pos = getpos()
			local crimpad = workspace["Criminals Spawn"].SpawnLocation
			crimpad.CanCollide = false
			crimpad.Transparency = 1
			repeat task.wait()crimpad.CFrame = getpos() until plr.Team == game.Teams.Zeykoz
			wait()
			MoveTo(pos)
			MoveToJunk(crimpad)
			return
	...
	Events = {
		...
		loadchar = function()
			...
			elseif plr.Team == game.Teams.Guards then
				...
				workspace.Remote.TeamEvent:FireServer(ohString1)
			elseif plr.Team == game.Teams.Zeykoz then
		...
		if Args[2] then
			if Args[2] == "all" then
				...
				if r  then
					local v = r
					local p = getpos()
					if v and v.Team ~= game.Teams.Guards and v.Team ~= game.Teams.Neutral and v ~=plr and BadArea(v) or v.Team == game.Teams.Zeykoz then
						local d= false
						task.spawn(function()
							repeat task.wait()
								pcall(function()
